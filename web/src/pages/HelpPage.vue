...
<template>
  <div class="help scroll">
    ...
    <section class="card">
      ...
      <p>每步选通货、动作前显式词缀数量、动作后稀有度和显式词缀数量，以及命中 / 未命中后的去向（下一步、重复、跳转、完成、停止）。</p>
      <p>动作前词缀数不符时不会消耗通货，而是直接用现有装备检查动作后条件，再走命中或未命中去向。</p>
    </section>
    ...
    <section class="card">
      ...
      <p>花园：整页一套规则。普通：每步各有一套；词缀条件留空时只判断配置的动作后稀有度和显式词缀数量。</p>
    </section>
